...
class Server(Node):
    def __init__(self):
        ...
        self.x_hat_now_minus = np.zeros((3,1))
        self.x_hat_now_plus = np.zeros((3,1)) 
        self.x_hat_last_plus = np.zeros((3,1))
        self.p_now_minus = np.diag(np.ones(3)*0.0025)
        self.p_now_plus = np.diag(np.ones(3)*0.0025)
        self.p_last_plus = np.diag(np.ones(3)*0.0025)
    ...
    def update(self):
        ...
        H = np.zeros((3, 3))
        H[0, 0] = (location[0] - L0[0]) / self.z_hat[0]
        H[0, 1] = (location[1] - L0[1]) / self.z_hat[0]
        H[1, 0] = (location[0] - L1[0]) / self.z_hat[1]
        H[1, 1] = (location[1] - L1[1]) / self.z_hat[1]
        H[2, 0] = (location[0] - L2[0]) / self.z_hat[2]
        H[2, 1] = (location[1] - L2[1]) / self.z_hat[2]

        K = self.p_now_minus @ H.T @ LA.inv(H @ self.p_now_minus @ H.T + self.R)

        self.p_now_plus = (np.eye(3) - K @ H) @ self.p_now_minus
    ...
    def step_calculation(self, wl, wr):
        ...
        self.x_hat_now_minus[2] = psi_last_plus + rot_head
